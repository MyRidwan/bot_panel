...
surat=$(curl -sS https://raw.githubusercontent.com/myridwan/izinvps/ipuk/ipmini | grep -w $ipes | awk '{print $4}')
...
EOF
    ...
    fun_bot1() {
      [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
        wget -qO- https://raw.githubusercontent.com/myridwan/bot_panel/ipuk/BotAPI.sh >/etc/.maAsiss/.Shellbtsss
      }
